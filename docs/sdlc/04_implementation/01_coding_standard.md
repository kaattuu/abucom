...
### 4.3 Larangan Class
Deklarasi `class` tidak boleh digunakan untuk memetakan objek bisnis.
...
Tidak boleh ada session global. Konteks seperti `db_conn` atau `jwt_payload` (yang berisi `branch_id`) wajib dikirimkan secara eksplisit (*passed as arguments*).
...
Sistem menggunakan **Layered Architecture** dengan alur *Top-Down*. Lapisan bawah tidak boleh memanggil lapisan di atasnya.
...
3. **Token Autentikasi**: Menggunakan JWT untuk otorisasi *stateless*.
   ...
   ```
4. **Manajemen Secret**: `DB_HOST`, `DB_USER`, `DB_PASS`, dan *Secret Key* wajib dibaca dari file lokal `.env` menggunakan `python-dotenv`. File `.env` dilarang disatukan ke Git.
5. **Audit Trail Append-Only**: Fungsi `log_audit()` akan menambahkan entri baru. Pemanggilan SQL `UPDATE` atau `DELETE` pada tabel log dilarang mutlak.
6. **Soft Delete**: Klausa `DELETE` dilarang untuk entitas operasional. Semua kueri modifikasi hapus harus berupa *update flag*.
   ```python
   # Contoh Soft Delete
   cursor.execute("UPDATE products SET is_deleted = 1 WHERE id = %s", (product_id,))
   ```
7. **SQL Role-Based Filtering**: Tambahkan `WHERE employee_id = %s` pada kueri sensitif jika *role* adalah `Karyawan`.

---

## 11. Penanganan Error

- **Stack Trace Hiding**: Terminal CLI pelanggan dan kasir dilarang menampilkan peringatan eror sintaks Python atau rincian basis data.
...
def execute_payment(db_conn, data: tuple) -> bool:
    try:
        ...
        db_conn.rollback()
        # TODO: catat 'e' di internal log — isi secara manual
        return False
```
...
Seluruh nilai uang dan harga dilarang dievaluasi menggunakan *floating-point* standar Python (`float`).
...
Sistem harus bisa beroperasi identik di OS Windows dan Linux (Debian). Tidak diperbolehkan menyisipkan literal garis miring lintasan berkas (`\` atau `/`). Gunakan fungsi murni standar `pathlib.Path()`.
...
- **Efisiensi Memori (RAM)**: Pemrosesan HPP tingkat berat tidak boleh menelan RAM melebihi **256 MB**.
...
- [ ] Modifikasi tabel selalu menggunakan filter wajib `is_deleted = 0`.
